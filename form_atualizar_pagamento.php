...
$sql4 = "SELECT v.ven_codigo, c.cli_nome, c.cli_codigo, p.pag_codigo, p.pag_tipo_pag
          FROM tb_vendas AS v
          INNER JOIN tb_clientes AS c ON v.cli_codigo = c.cli_codigo
...
$sql3 = "select * 
        ...
        inner join tb_vendas as v on p.ven_codigo = v.ven_codigo
        where v.ven_codigo =  '$venda_id'";
                    ...
                    <div class="col-xl-12 col-xxl-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Formulário de Atualização de Pagamento</h4>
                            </div> 

                            <div class="card-body">
                                <div class="basic-form">

                                <form name="frm_pagamento" method="post" action="atualizar/atualizar_pagamento.php" onsubmit="return validarFormularioPagamento();">
                                    <div class="col-lg-12 mb-4">
                                        <label>Código</label>
                                        <input type="text" class="form-control" placeholder="" readonly name="txt_venda" id="txt_codigo" value="<?php echo $venda_id; ?>">                                           
                                    </div>
                                    ...
                                    <div class="col-lg-12 mb-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="txt_descricao" id="txt_descricao" placeholder="Digite uma breve descrição" value="<?php echo $dados3['pag_descricao']; ?>">
                                        </div>
                                    </div>  
                                    ...
                                    <div class="col-lg-12 mb-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="txt_tipo_pag" id="tipo_pagamento" required>
                                                <option value="<?php echo $dados4['pag_tipo_pag']; ?>"> <?php echo $dados4['pag_tipo_pag']; ?> </option>
                                                    <?php while ($dados5 = mysqli_fetch_array($sql5)) { ?>
                                                        <option value="<?php echo $dados5['pag_tipo_pagamento']; ?>"><?php echo $dados5['pag_tipo_pag']; ?></option>
                                                    <?php } ?>
                                            </select>
                                    ...
                                    <script>
                                        document.getElementById('tipo_pagamento').addEventListener('onblur', function() {
                                            var tipoPagamento = this.value;
                                            var dataVencimento = document.getElementById('data_vencimento');
                                            dataVencimento.innerHTML = ''; // Limpa os campos anteriores
                                            document.getElementById('parcelas_div').style.display = 'none'; // Esconde parcelas por padrão
                                            document.getElementById('datas_parcelas_div').innerHTML = ''; // Limpa datas de parcelas
                                            document.getElementById('valores_parcelas_div').innerHTML = ''; // Limpa valores de parcelas

                                            if (tipoPagamento === '3') {
                                                document.getElementById('parcelas_div').style.display = 'block'; // Mostra campo de parcelas
                                            } else if (tipoPagamento === '0') {
                                                // Não faz nada, campos já foram limpos
                                            } else {
                                                // Para Dinheiro e Cartão, cria um campo de data única
                                                ...
                                                input.type = 'date';
                                                input.name = 'txt_data_vencimento'; // Apenas uma data
                                        ...
                                        function mostrarCamposParcelas() {
                                            ...
                                            var totalValor = parseFloat(document.getElementById('txt_total').value) || 0; // Obtém o valor total
                                            var valorParcela = (totalValor / numParcelas).toFixed(2); // Calcula o valor de cada parcela
                                            ...
                                            datasParcelasDiv.innerHTML = ''; // Limpa os campos anteriores
                                            valoresParcelasDiv.innerHTML = ''; // Limpa os campos anteriores
                                            ...
                                            for (var i = 1; i <= numParcelas; i++) {
                                                ...
                                                inputData.type = 'date';
                                                inputData.name = 'txt_data_vencimento[]'; // Array para múltiplas datas
                                                ...
                                                inputValor.name = 'txt_valor[]'; // Array para múltiplos valores
                                                inputValor.className = 'form-control';
                                                inputValor.value = valorParcela; // Define valor padrão
                                        ...
                                        function verificarTotal(event) {
                                            event.preventDefault(); // Impede o envio do formulário até a verificação

                                            var valores = document.querySelectorAll('input[name="txt_valor[]"]');
                                            var totalInput = document.getElementById('txt_total');
                                            var totalValor = parseFloat(totalInput.value) || 0;
                                            var somaParcelas = Array.from(valores).reduce((acc, input) => {
                                                return acc + (parseFloat(input.value) || 0);
                                            }, 0);
                                            
                                            if (somaParcelas !== totalValor) {
                                                // Usando SweetAlert
                                                swal({
                                                    title: "Erro!",
                                                ...
                                                // Se a soma estiver correta, envie o formulário
                                        ...
                                        function validarFormularioPagamento() {
                                            ...
                                            // Validação adicional para o tipo de pagamento 'Crédiario'
                                            if (tipoPagamento.value === "3") {
                                                const numParcelas = document.getElementById("num_parcelas").value;
                                                const datasParcelas = document.querySelectorAll('input[name="txt_data_vencimento[]"]');
                                                const valoresParcelas = document.querySelectorAll('input[name="txt_valor[]"]');
                                                
                                                if (numParcelas === "" || parseInt(numParcelas) < 1) {
                                                    destacarCampo(document.getElementById("num_parcelas"));
                                                } else {
                                                    removerDestaque(document.getElementById("num_parcelas"));
                                                }

                                                // Verificar se todas as datas de vencimento estão preenchidas
                                                datasParcelas.forEach((data) => {
                                                    if (data.value === "") {
                                                        destacarCampo(data);
                                                    } else {
                                                        removerDestaque(data);
                                                    }
                                                });

                                                // Verificar se todos os valores das parcelas estão preenchidos
                                                valoresParcelas.forEach((valor) => {
                                                    if (valor.value === "" || parseFloat(valor.value) <= 0) {
                                                        destacarCampo(valor);
                                                    } else {
                                                        removerDestaque(valor);
                                                    }
                                                });
                                        ...
                                        document.getElementById('btn_salvar1').addEventListener('click', function(event) {
                                            ...
                                            if (validarFormularioPagamento()) {
                                                ...
                                                if (tipoPagamento === '3') {
                                                    verificarTotal(event); // Verifica o total apenas se for crediário
                                                } else {
                                                    document.forms['frm_pagamento'].submit(); // Submete o formulário para outros tipos de pagamento
                                                }
                                            }
                                        });
